...
class DetectionEvaluator:
    ...
    A detector should have this simplified interface:
        Y_pred = detector(X)
    """
    def __init__(self, model, result_folder, csv_fname, dataset_name, device):
        pass
        # set_base_model()
        self.model = model
    ...
    def build_detection_dataset(self, X, Y_label, Y_pred, selected_idx, X_adv_list, Y_adv_pred_list, attack_names, attack_string_hash, clip, Y_test_target_next, Y_test_target_ll):
        ...
        Y_label_adv = Y_label[:len(X_adv_all)]

        detection_db_path = os.path.join(self.task_dir, "detection_db_%s_clip_%s.json" % (attack_names[0] + self.dataset_name, clip))
    ...
    def get_detector_by_name(self, detector_name):
        ...
        if detector_name.startswith('FeatureSqueezing'):
            detector = FeatureSqueezingDetector(model, detector_name, self.dataset_name, self.device)
    ...
    def evaluate_detections(self, params_str):
        ...
        for detector_name in detector_names:
            ...
            if isinstance(X_fae, np.ndarray):
