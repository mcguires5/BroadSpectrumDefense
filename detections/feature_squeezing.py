...
class FeatureSqueezingDetector:
    def __init__(self, model, param_str, dataset_name, device):
        self.model = model
    ...
    def get_distance(self, X1, X2=None):
        ...
        input_to_normalized_output = lambda x: normalize_func(reshape_2d(self.eval_layer_output(x)))
        import cv2
        ...
        if X2 is None:
            ...
            for squeezer_name in squeezers_name:
                ...
                mod_X1 = squeeze_func(X1)
                val_squeezed_norm = input_to_normalized_output(mod_X1)
    ...
    def eval_layer_output(self, X):
        ...
        s = Softmax(dim=1)
        if X.shape[1] != 3 and (self.dataset_name == 'CIFAR10' or self.dataset_name == 'CIFAR100' or self.dataset_name == 'TinyImageNet' or self.dataset_name == 'ImageNet'):
            X = np.moveaxis(X, 3, -3)
        print(np.min(X))
        print(np.max(X))
        preds = []
        for i in range(0, len(X), 64):
            preds.append(s(self.model(torch.as_tensor(X[i:i + 64]).to(self.device))).detach().cpu().numpy())
