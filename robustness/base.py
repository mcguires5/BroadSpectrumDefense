...
class RobustClassifierBase:
    def __init__(self, model, rc_name):
        self.model_predict = lambda x: model(x)
    def predict(self, X):
...
def evaluate_robustness(params_str, model, Y, X, Y_adv, attack_string_list, X_adv_list, fname_prefix, selected_idx_vis, result_folder):
    ...
    for RC_name in RC_names:
        ...
        img_fpath = os.path.join(result_folder, '%s_%s.png' % (fname_prefix, RC_name) ).replace('?','_')
        ...
        for row in accuracy_rows:
            if best_acc < row[attack_name]:
                best_acc = row[attack_name]
                best_legit_acc = row['legitimate_%d' % len(X)]
    ...
    return best_acc, attack_name, best_legit_acc