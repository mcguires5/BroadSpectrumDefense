...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            if batch_idx % 50 == 0:
                ...
                print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}\tLR: {:.6f}'.format(epoch,
                    batch_idx * len(img),
                    len(dataloader.dataset), 50. * batch_idx / len(dataloader),
        ...
        pwd = os.getcwd()
        save_dir = os.path.join(pwd, 'Autoencoders/' + str(dataset) + '/' + str(epoch) + "ep_AE_TinyImageNetAEStruct_0.05_noise_-1to1MSE.pth")
        if epoch == num_epochs or epoch == num_epochs/2:
