...
class StandardMNIST(nn.Module):
    ...
    def forward(self, x):
        ...
        x_pred = self.softmax((self.fc3(x)))
        return x_pred, x
...
mnist_trainset = datasets.MNIST(root='./Datasets', train=True, download=False, transform=transforms.Compose([transforms.ToTensor()]))
mnist_testset = datasets.MNIST(root='./Datasets', train=False, download=False, transform=transforms.Compose([transforms.ToTensor()]))
